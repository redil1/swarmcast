...
import { selectTrackerShard } from "../services/tracker/src/sharding.js";
...
async function joinOnShard({ state, selfShardId, fetchFn }) {
  const peer = createPeer({ sub: `viewer-${selfShardId}` });
  const ws = createWs();
...
assert.equal(ownerState.swarms.get(channelId).size, 1);
...
console.log(`tracker sharding smoke OK: channel=${channelId} owner=${target.id} redirectFrom=${wrongShard.id} demandCalls=${demandCalls.length}`);

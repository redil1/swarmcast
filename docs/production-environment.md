...
- segment metadata bus evidence covering a three-node TLS cluster across real failure domains, three replicas, subject permission denial, one-node loss, persistent latest replay, credential rotation, peak publish/delivery latency, and storage recovery; local single-node coverage is `npm run smoke:segment-bus` and local three-node behavior coverage is `npm run smoke:segment-bus-cluster`

...
- `npm run smoke:ingest-demand-playlist`: starts a real ingest HTTP server with `ChannelManager`, demands a channel, publishes the committed real H.264/AAC fMP4 regression bytes through a worker shim, structurally validates and watches the segment, announces it to a fake tracker internal endpoint, and verifies live status.

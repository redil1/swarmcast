...
- Android P2P transfer evidence follows `docs/android-device-lab.md` and passes `npm run android:p2p:evidence:validate -- path/to/android-p2p-evidence.json`. It requires at least four distinct Play-installed physical devices across two WiFi failure domains and two cellular carriers, a 30-minute soak, complete per-device ICE outcomes, verified segment hashes, measured WiFi useful upload and cellular zero upload, P2P-disable closure with edge fallback, direct `rho >= 0.90`, and reconciled tracker, edge, origin-bootstrap, and relay egress. Raw device-lab output is not launch evidence until those server/provider records and independent reviews are joined; local guard coverage remains `npm run smoke:android-device-lab` and `npm run smoke:android-p2p-evidence-validation`.
...
- Segment metadata bus evidence follows `docs/segment-bus-capacity.md` and passes `npm run segment-bus:capacity:evidence:validate -- path/to/segment-bus-capacity-evidence.json`. It requires a hash-bound mode-`0600` raw probe from exactly three real failure domains across at least two providers, sustained projected peak publication plus headroom, exact publish/delivery reconciliation, leader-loss quorum, persistent latest replay after full-cluster restart, permission denial, credential rotation, TLS and route mTLS, storage/monitoring reconciliation, and distinct platform, performance, and security approvals. Local guard coverage remains `npm run smoke:segment-bus-capacity-probe`, `npm run smoke:segment-bus-capacity-evidence-validation`, and `npm run smoke:segment-bus-cluster`; synthetic fixtures are not launch evidence.
- Capacity/load ladder evidence follows `docs/distributed-load-ladder.md`, uses `npm run load:ladder:probe` on independent generator hosts, and passes `npm run capacity:plan:validate -- config/capacity-plan.json` without `--allow-draft` plus `npm run load:ladder:validate -- path/to/load-ladder-evidence.json`. It requires hash-bound mode-`0600` raw probes, exact peer-range coverage, synchronized cross-provider starts, measured cross-host WebRTC DataChannels and direct-P2P offload, measured sustained TLS edge throughput, approved provider traffic terms, relay egress accounting, the self-sustaining sweep, and 1K/10K/100K single-channel cell stages; local guard coverage remains `npm run smoke:capacity-plan-validation`, `npm run smoke:load-ladder-probe`, and `npm run smoke:load-ladder-evidence-validation`.
...
| Image scan reports | TBD | Partial | `npm run image:scan:bundle:validate` enforces one clean scan report per digest-pinned release image, and launch evidence must reference all 16 service and infrastructure scan report paths; real production scan artifacts remain open |
...
| Segment metadata bus capacity | TBD | Blocked | The exact three-domain runner and evidence validator are implemented; a real 15-minute staging run, artifact hash, monitoring reconciliation, and three independent approvals remain open |
...
The launch record must bind the artifact bundle path and SHA-256. The bundle must contain the exact 53-artifact inventory for all 34 gates; every artifact is hash-checked and passed to one of 38 repository-owned fixed validator groups. The record also requires distinct release, operations, and security approvals after artifact generation. Evidence files cannot select validator commands.

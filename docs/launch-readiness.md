...
| Android CI build | TBD | Partial | Local debug/release assembly passed with Android SDK platform 35 and the Gradle 8.9 wrapper; Android release properties are shape-gated by `npm run android:release-config:validate`; CI now uploads debug and unsigned release APK artifacts with checksum sidecars, and first remote CI evidence is still required |

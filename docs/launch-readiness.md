...
- Capacity plan passes `npm run capacity:plan:validate -- config/capacity-plan.json` with measured `rho`, cache hit ratio, peak viewers, bitrate, and node capacity.
...
| Android CI build | Release Engineering | Ready | Private GitHub Actions run `29327610922` passed Android unit tests and debug/release assembly at commit `8685f81207499c3e2f843f9755f1d0b5374ee7d6`; both downloaded artifacts passed their portable SHA-256 sidecars, and `evidence/android/ci-build-29327610922.json` passes `npm run android:ci:evidence:validate` |
...
| Image scan reports | TBD | Partial | `npm run image:scan:bundle:validate` enforces one clean scan report per digest-pinned release image, and launch evidence must reference all 12 service and infrastructure scan report paths; real production scan artifacts remain open |

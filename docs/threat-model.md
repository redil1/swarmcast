...
| T-018 | A forged, stale, aliased, symlinked, or validator-substituted evidence bundle is approved as launch-ready. | Schema-version-2 launch records bind a mode-0600 bundle by SHA-256; the bundle binds the exact 53-artifact inventory by SHA-256, rejects aliases/symlinks/test fixtures, executes 38 fixed validator groups without a shell, and requires distinct release, operations, and security approvals after generation. | Use only immutable real artifacts and complete the three-person approval on the final generated bundle. |

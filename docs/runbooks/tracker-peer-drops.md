...
4. Compare `swarmcast_tracker_join_timeouts_5m` with peer count, tracker CPU, event-loop latency, and auth latency by cell.
5. Confirm control-plane placement and ingest demand routes are responding.
6. Check whether drops or join timeouts are concentrated on one app version, network type, region, channel, or tracker cell.
7. Reproduce the tracker restart path with `npm run smoke:tracker-ws-restart`; on a local Node 24 shell, use `TRACKER_WS_DOCKER_IMAGE=swarmcast-tracker:local npm run smoke:tracker-ws-restart`.
...
- Capture join-timeout counts and rates by tracker cell before and after mitigation.
